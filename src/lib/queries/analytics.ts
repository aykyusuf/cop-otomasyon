...
import {
  getMockCollectionFrequency,
  getMockDashboardStats,
  getMockFillTrends,
  getMockWasteTypeDistribution,
  getMockZoneEfficiency,
} from "@/lib/mock-db";
...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  try {
    [binsResult, alertsResult, collectionsResult] = await Promise.all([
      query(`
        SELECT
          COUNT(*) as total,
    ...
    if (isDatabaseConnectionError(error)) return getMockDashboardStats();
    throw error;
  }
...
export async function getFillTrends(hours: number = 24) {
  try {
    ...
    if (isDatabaseConnectionError(error)) return getMockFillTrends(hours);
    throw error;
  }
}
...
export async function getCollectionFrequency(days: number = 7) {
  try {
    ...
    if (isDatabaseConnectionError(error)) return getMockCollectionFrequency(days);
    throw error;
  }
}
...
export async function getWasteTypeDistribution() {
  try {
    ...
    if (isDatabaseConnectionError(error)) return getMockWasteTypeDistribution();
    throw error;
  }
}
...
export async function getZoneEfficiency() {
  try {
    ...
    if (isDatabaseConnectionError(error)) return getMockZoneEfficiency();
    throw error;
  }
}
